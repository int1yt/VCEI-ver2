...
Env:
  ...
  CAN_PRETRAINED_PATH  directory with ckpt_epoch_*.pth and ckpt_class_epoch_*.pth (used only if CarHack model missing)
...
from features import can_packets_to_matrix_29x29, eth_packets_to_sequence_10x80
...
def health() -> Dict[str, Any]:
    ...
    return {
        ...
        "can_backend": "carhack_cnn" if ch.model is not None else "supcon_or_stub",
        ...
        "hint": "See MODELS.md if eth_model_loaded is false. CAN uses CarHack CNN if carhack_can_loaded else SupCon path.",
    }
...
def enrich(body: EnrichRequest) -> Dict[str, Any]:
    ...
    # CAN ML — prefer CarHackData-trained CNN if weights exist
    ...
    if can_hist:
        ...
        can_ml = {
            ...
            "confidence": conf,
            **{k: v for k, v in meta.items() if k != "probs"},
        }
        if "probs" in meta:
            can_ml["class_probs"] = {
                CAN_CLASS_NAMES[i]: float(meta["probs"][i]) for i in range(min(len(CAN_CLASS_NAMES), len(meta["probs"])))
            }
    ...
    return {
        ...
        "ethernet_ml": eth_ml,
        "can_ml": can_ml,
        "can_class_names": CAN_CLASS_NAMES,
        "eth_model_note": (
